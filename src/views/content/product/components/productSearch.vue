...
const emit = defineEmits(['onPublish', 'onSearch'])
// 批量操作变量
const status = ref()
const keyword = ref('')
const addProduct = () => UseCommon.ins().goUrlName('addProduct')
const onClassify = () => {
  UseCommon.ins().goUrlName('productClassify', { type: 'product' })
}
const onSearch = () => emit('onSearch', keyword.value)
// 批量操作
const handleChange = () => {
  ...
  if (status.value < 2) {
    ...
    Utilts.ins().confirm({
      okCallBack: () => {
        API.product.postProductDelete(params).then((res) => {
          if (res.data) {
...
<template>
  <div class="flex justify-between">
    <div class="flex justify-start items-center space-x-4">
      <CommonSearch v-model:keyword="keyword" tip="请输入产品名称" @on-search="onSearch"></CommonSearch>
      <a-button @click="addProduct" class="flex items-center" type="primary">
        <template #icon>
          <PlusOutlined class="text-sm" />
        </template>
        添加产品
      </a-button>
      <a-button class="items-center hidden" @click="onClassify">产品分类</a-button>
    </div>
    <div class="flex justify-start items-center space-x-4">
      <a-button hidden class="flex items-center" size="small">
        <template #icon>
          <DownloadOutlined />
        </template>
        导入
      ...
      <a-select class="w-[120px]" size="large" placeholder="批量操作" ref="select" v-model:value="status"
        @select="handleChange">
